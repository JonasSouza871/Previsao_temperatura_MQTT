...
static bool calcular_regressao_linear(float *dados_x, float *dados_y, int n, float *m, float *b) {
    if (n < 2) {  // Verifica se há pontos suficientes para a regressão
        *m = 0.0;
        *b = (n > 0) ? dados_y[0] : 0.0;
        return false;
    }
...
static float calcular_previsao_temperatura(float tempo_atual) {
    float m, b;
    int numero_pontos = previsao_temperatura.historico_preenchido ?
                     TAMANHO_HISTORICO_TEMP : previsao_temperatura.indice_historico;

    // Tenta calcular a regressão linear se houver pontos suficientes
    if (numero_pontos >= 2 &&
        calcular_regressao_linear(previsao_temperatura.historico_tempo,
                                  previsao_temperatura.historico_temperatura,
                                  numero_pontos, &m, &b)) {
...
static void tarefa_temperatura(void *pvParameters) {
    ...
    while (1) {
        ...
        if (leitura_temperatura > -20.0 && leitura_temperatura < 80.0) {
            ...
            float tempo_atual = (float)(xTaskGetTickCount() - tempo_inicial_ticks)
    ...
    while (1) {
        ...
        if (estado_sistema.tela_atual == 0) {
            if ((tick_atual - ultima_mudanca_joystick) > pdMS_TO_TICKS(DEBOUNCE_JOYSTICK_MS)) {
                ...
                if (valor_adc > 3000) {  // Joystick para cima
                    comando_usuario.tipo = COMANDO_AJUSTAR_URGENCIA_SUBIR;
                    comando_usuario.valor = 1;
                    mudou_temp = true;
                } else if (valor_adc < 1000) {  // Joystick para baixo
...
static void tarefa_display(void *pvParameters) {
    ...
    while (1) {
        ...
        xTaskNotifyWait(0x00, ULONG_MAX, &valor_notificacao,
                       pdMS_TO_TICKS(TIMEOUT_ATUALIZACAO_DISPLAY_MS));
